...
{
    public abstract class ModifyMarketplaceItem : CoreModify<UserItem>
    {
        ...
        public override async Task<IList<IEvent>> PerformModify(bool isNew, string id, NHibernate.ISession session)
        {
            ...
            {
                dbItem = session.Get<UserItem>(id);
            }
            ...
            if (fileInfo != null && fileInfo.Data != null && fileInfo.Data.Length > 0)
            {
                ...
                var existingFiles = session.QueryOver<FileItem>().Where(x => x.UserItem.Id == dbItem.Id).List().ToList();
